...
CREATE OR REPLACE PACKAGE BODY PKG_VENTAS
	...
	AS
  	BEGIN
    	OPEN FILAS FOR
    	SELECT ID_PRODUCTO AS CODIGO, NOMBRE AS PRODUCTO, IMP_PRODUCTO, TARIFA_IMP, DESC_PRODUCTO AS DESCUENTO, PRECIO_UNITARIO, DESCRIPCION AS CATEGORIA, PROVEEDOR 
	...
	AS
  	BEGIN
    	OPEN FILAS FOR
    	SELECT ID_PEDIDO AS #_PEDIDO, NOMBRE || ' ' || APELLIDOS  AS NOMBRE_CLIENTE, TOTAL, FECHA
    	FROM CLIENTES,PEDIDO 
    	WHERE CLIENTES.ID_CLIENTE = PEDIDO.ID_CLIENTE AND PEDIDO.FECHA= VFECHA;
  	END;
	
	FUCTION CIERRE_CAJA (MFECHA VARCHAR2);
